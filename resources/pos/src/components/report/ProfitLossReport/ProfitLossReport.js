...
const ProfitLossReport = (props) => {
    ...
    return (
                ...
                <Col className="col-12 mb-4">
                    <Row className={"align-items-start"}>
                        ...
                        <ProfitLossWidget
                            ...
                            allConfigData={allConfigData}
                            moreText={`Sales Profit: (${currencySymbolHendling(
                                allConfigData,
                                frontSetting.value &&
                                    frontSetting.value.currency_symbol,
                                profitAndLossReport.sales
        ...
        Returns: (${currencySymbolHendling(
            allConfigData,
                ...
                : "0.00"
                            ...
                            value={(() => {
                                const sales = parseFloat(
                                    profitAndLossReport.sales || 0
                                );
                                const returns = parseFloat(
                                    profitAndLossReport.sale_returns || 0
                                );
                                const productCost = parseFloat(
                                    profitAndLossReport.product_cost || 0
                                );

                                // Calculate the ratio of returns to sales
                                const returnRatio = returns / sales;

                                // Calculate cost proportional to actual sales (excluding returns)
                                const effectiveCost =
                                    productCost * (1 - returnRatio);

                                // Final profit calculation
                                const grossProfit =
                                    sales - returns - effectiveCost;
                        ...
                        <ProfitLossWidget
                            ...
                            value={(() => {
                                const paymentRecived = profitAndLossReport.payments_received
                                    ? parseFloat(profitAndLossReport.payments_received).toFixed(2)
                                    : "0.00";
