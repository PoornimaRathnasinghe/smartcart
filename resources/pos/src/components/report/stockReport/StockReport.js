...
const StockReport = (props) => {
    ...
    const itemsValue =
        ...
        (() => {
            const items = stockReports.map((stockReport) => ({
                code: stockReport.attributes.product.code,
                name: stockReport.attributes.product.name,
                size: stockReport.attributes.product.size, // Add this line
                product_category_name:
                    stockReport.attributes.product_category_name,
                ...
                total_cost_price:
                    stockReport.attributes.product.product_cost *
                    stockReport.attributes.quantity,
            ...
            const grandTotal = items.reduce(
                (total, item) => total + item.total_cost_price,
                0
            );
            ...
            items.push({
                code: "",
                name: "",
                size: "", // Add empty size for total row
                product_category_name: "",
                product_cost: "",
                product_price: "",
                product_unit: "",
                current_stock: "",
                total_cost_price: grandTotal,
                id: "grand_total",
                currency: currencySymbol,
                isTotal: true,
            });
    ...
    const columns = [
        ...
        },
        {
            ...
            selector: (row) => (row.isTotal ? "" : row.name),
            sortField: "name",
            sortable: false,
        },
        {
            name: "Size", // Add size column after name
            selector: (row) => (row.isTotal ? "" : row.size),
            sortField: "size",
            sortable: false,
            cell: (row) => {
                if (row.isTotal) return "";
        ...
        },
        {
            ...
            selector: (row) => (row.isTotal ? "" : row.product_category_name),
        ...
        {
            name: getFormattedMessage("Total Cost Price"),
            sortField: "total_cost_price",
            sortable: false,
            cell: (row) => {
                if (row.isTotal) {
                    return (
