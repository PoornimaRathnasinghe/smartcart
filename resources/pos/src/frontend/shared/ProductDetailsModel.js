...
const ProductDetailsModel = (props) => {
    ...
    const [orderTax, setOrderTax] = useState(
        parseFloat(product.tax_value).toFixed(2)
    );
                ...
                productUnits
                    .filter(
                        (item) =>
                            Number(item.id) ===
                            Number(
                                product.sale_unit && product.sale_unit.value
                                    ? product.sale_unit.value
                                    : product.sale_unit
                            )
                    )
                    .map((item) => {
    ...
    useEffect(() => {
        ...
        setTaxType(
            product.tax_type === 1 || product.tax_type === "1"
                ? {
                      value: 1,
    ...
    const onBlurTax = (e) => {
        const { value } = e.target;
        // Update orderTax to 0.00 if the input is empty or invalid
    ...
    const discountAmount = (totalCost) => {
        let dis = 0;
        if (
            (discount > 0 && discountType.value === "2") ||
            discountType.value === 2
        ) {
            dis = Number(discount);
        } else if (
            (discount > 0 && discountType.value === "1") ||
            discountType.value === 1
        ) {
            const percentDiscount =
                discountType.value === "1" || discountType.value === 1
                    ? (parseFloat(totalCost).toFixed(2) * Number(discount)) /
                      Number(100)
                    : 0;
            dis = +percentDiscount;
        }
    ...
    //         tax = +totalCost;
    //     } else if (
    //         (orderTax > 0 && taxType.value === "1") ||
    //         taxType.value === 1
    //     ) {
    //         let exclusiveTax =
    //             taxType.value === "1" || taxType.value === 1
    //                 ? (parseFloat(total).toFixed(2) * Number(orderTax)) /
    //                   Number(100)
    //                 : 0;
    //         tax = +exclusiveTax;
    //     }
    ...
    const taxAmount = (totalCost) => {
        const total = totalCost - discountAmount(product.product_price);
        let tax = 0;
        const parsedOrderTax = isNaN(parseFloat(orderTax))
            ? 0
            : parseFloat(orderTax);
        if (
            (parsedOrderTax > 0 && taxType.value === "2") ||
            taxType.value === 2
        ) {
            tax = +totalCost;
        } else if (
            (parsedOrderTax > 0 && taxType.value === "1") ||
            taxType.value === 1
        ) {
            let exclusiveTax =
                taxType.value === "1" || taxType.value === 1
                    ? (parseFloat(total).toFixed(2) * parsedOrderTax) /
                      Number(100)
                    : 0;
            tax = +exclusiveTax;
        }
    ...
    const onSaveDetailModal = () => {
        ...
        if (Valid) {
            if (productModelId === product.id) {
                ...
                newProduct.tax_type = taxType.value;
                newProduct.sale_unit = saleUnitType[0]
                    ? saleUnitType[0].value
            ...
            updateCost(
                (newProduct.net_unit_cost = calculateProductCost(unitPrice))
            );
